...
{   
    ...
    if ( !fFound )
        throw runtime_error("Error: The given transaction contain no claim scripts owned by this wallet");
...
{   
    ...
    CReserveKey reserveKey(pwalletMain);
    CAmount nFeeRequired;
    if ( !pwalletMain->AbandonCash( vecSend,wTxNew,reserveKey,nFeeRequired,nChangePosRet,strError, NULL, true, &wTxIn, nTxOut))
    {   
...
void ListNameClaims(const CWalletTx& wtx,const string &strAccount,int nMinDepth,UniValue &ret,const isminefilter & filter,bool list_spent)
{   
    CAmount nFee;
            ...
            {   
                ...
                if ( it != mapBlockIndex.end() )
                {
                    ...
                    if ( pindex )
                    {
                        ...
                        {
                            entry.push_back(Pair("expired",true));              //expired blocks
                        }
                    }
                }
